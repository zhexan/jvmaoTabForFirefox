import { storage, runtime, contextMenus, tabs } from "../newtab/utils/browser-polyfill";
...
const setOption = (sendResponse, values) => {
  ...
  setTimeout(() => {
    ...
    if (Object.keys(saveOptionsValue).length > 0) {
      ...
      saveOptionsValue = {};
      storage.sync.set(dataToSave, function (res) {
        getOptionTime = 0;
      });
    }
  }, 1000);
...
const _getOption = () => {
  return new Promise((resolve, reject) => {
    if (getOptionTime + 10 * 60 * 1000 > Date.now()) {
      ...
      storage.sync.get(cloudOptions, function (result) {
        optionsValue = result;
...
const setContextMenusData = (type, data, tab) => {
  ...
  storage.local.get(['contextMenusData'], function (res) {
    let saveData = [];
...
contextMenus.onClicked.addListener(function (info, tab) {
  if (info.menuItemId === "imageMenu") {
    ...
    tabs.sendMessage(tab.id, {
      ...
      if (runtime.lastError || !response?.html) {
        // 如果没有收到消息，默认走info
        setContextMenusData('text', info, tab);
      } else {
        // 正常处理响应
